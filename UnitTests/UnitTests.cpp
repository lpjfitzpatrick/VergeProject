...
namespace UnitTests
{
	...
	public:
		...
		{
			// Invalid. Return -2.0 as an arbitrary error code
			std::vector<std::vector<double>> test{ };
		...
		{
			// Invalid. Return -1.0 as an arbitrary error code
			std::vector<std::vector<double>> test{
				{ 0.0 } };
		...
		TEST_METHOD(TenByTen)
		{
			...
			// minPath won't find the optimal route but it will find a feasible one (nodes 1,3,9,5,6)
			std::vector<std::vector<double>> test{
		...
		{
			// Modified 10x10 from the last step (1,5) and (5,1) are changed from 9.5 -> 1.2
			// This configuration would produce a spanning tree if the algorithm did not keep track of the last
			// visited node and where our current node has come from (which would give a different total weight)
			// We expect to get the same total weight as in the last test since we still start from node 1 and
		...
		TEST_METHOD(TwentyByTwenty)
		{
			...
			// (I'm going to use numbers < 10.0 so that the spacing will be consistent and I can eyeball the path easier)
			std::vector<std::vector<double>> test{
